...
def load_latest_checkpoints(load_dir, args, logger=logging):
    ...
    pattern = r'^((?!best_)(?!optimizer_))(?P<model_name>.+)\+(?P<jitter>.+)\+i(?P<in_features>.+)_' + \
              r'(?P<custum_datasets>.+)_(?P<i_iter>[\d]{6})\.pth$'
...
def save_checkpoints(save_dir, options, i_iter, model_dict, optimizer_dict, logger=logging, best=False):
...
def save_check(save_dir, i_iter, model_dict, optimizer_dict, best=False):
    if best:
        # print('save best model')
        save_path = os.path.join(save_dir, 'best_model.ckpt')
        # print(save_path)
        model_dict.update({'iteration': i_iter})
        torch.save(model_dict, save_path)
        if optimizer_dict:
            save_path = os.path.join(save_dir, 'best_optimizer.ckpt')
            if (os.path.isfile(save_path)):
                # print('remove previous weight')
                os.remove(save_path)
            optimizer_dict.update({'iteration': i_iter})
            torch.save(optimizer_dict, save_path)
    else:
        ...
        torch.save(model_dict, save_path)
        if optimizer_dict:
            save_path = os.path.join(save_dir, '%d_optimizer.ckpt' % (i_iter))
            optimizer_dict.update({'iteration': i_iter})
            torch.save(optimizer_dict, save_path)
