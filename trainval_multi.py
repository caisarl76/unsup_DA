...
def parse_args(args=None, namespace=None):
    ...
    # model options
    parser.add_argument('--num-classes', help='number of classes', default=10, type=int)
    parser.add_argument('--source-datasets', help='source training dataset', default=['Clipart'],
                        nargs='+')
    parser.add_argument('--merge-sources', help='Use merged dataset as source dataset.', action='store_true')
    parser.add_argument('--target-datasets', help='target training dataset', default=['RealWorld'], nargs='+')
    ...
    parser.add_argument('--batch-size', help='batch_size', default=20, type=int)
    ...
    for i_iter in range(start_iter, args.early_stop_step + 1):
        ...
        if i_iter % 2 == 0 and i_iter != 0:
            ...
            for trg_idx, target_val_dataloader_iter in enumerate(target_val_dataloader_iters, num_source_domains):
                ...
                for c, c_name in enumerate(target_val_datasets[trg_idx - num_source_domains].classes):
                    print('-------------------', c, c_name)
            ...
            if val_accuracy > best_accuracy:
                ...
                io_utils.save_checkpoints(args.save_dir, options, i_iter, model_dict, optimizer_dict, logger, best=True)
                # ship to cuda
                model = model.cuda(args.gpu)
                if args.adv_loss:
                    discriminators = [discriminator.cuda(args.gpu) for discriminator in discriminators]
