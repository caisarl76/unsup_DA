...
def resnet18dsbn(pretrained=False, **kwargs):
    ...
    """
    model = rot_DSBNResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet34dsbn(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = rot_DSBNResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
...
def resnet50dsbn(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = rot_DSBNResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
...
def _update_initial_weights_dsbn(state_dict, num_classes=4, num_domains=2, dsbn_type='all'):
...
class rot_DSBNResNet(nn.Module):
    def __init__(self, block, layers, in_features=256, num_classes=4, num_domains=2):
        self.inplanes = 64
        self.in_features = in_features
        self.num_domains = num_domains
        self.num_classes = num_classes
        super(rot_DSBNResNet, self).__init__()
...
def get_rot_model(model_name, num_domains=2):
    if(model_name == 'resnet18dsbn'):
        model = resnet18dsbn(pretrained=True, num_domains=num_domains)
